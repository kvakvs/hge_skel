...
MyGame::MyGame()
        : m_hge(NULL), m_font(NULL), m_mouse_cursor_sprite(NULL), m_state(NULL), m_sound_enabled(true) {
...
bool MyGame::Startup() {
  ...
  m_background_quad.blend = (hgeBlendMode)(BLEND_ALPHABLEND | BLEND_COLORMUL | BLEND_NOZWRITE);

  for (auto & i : m_background_quad.v) {
    // Set up z-coordinate of vertices
    i.z = 0.5f;
    // Set up color. The format of DWORD col is 0xAARRGGBB
    i.col = ARGB(0xFF, 0xFF, 0xFF, 0xFF).argb;
  }
