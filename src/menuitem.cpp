...
hgeGUIMenuItem::hgeGUIMenuItem(int _id, hgeFont *_fnt, HEFFECT _snd, float _x, float _y, float _delay,
                               const char *_title) {
  float w;
  ...
  w = fnt->GetStringWidth(title.c_str());
...
void hgeGUIMenuItem::Render() {
  ...
  fnt->Render(rect.x1 + offset + 3, rect.y1 + 3, HGETEXT_LEFT, title.c_str());
  fnt->SetColor(color.GetHWColor());
  fnt->Render(rect.x1 - offset, rect.y1 - offset, HGETEXT_LEFT, title.c_str());
}
